...
export default function VocabPage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-white">
      {/* Header */}
      <header className="flex items-center justify-between px-section pt-6 pb-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[12px] text-[#9f9fa9] leading-[18px]">
        ...
        <div className="flex items-center gap-2">
          {/* View toggle */}
          <div className="flex items-center bg-[#f4f4f5] rounded-[10px] h-[30px] p-0.5">
            <button
              onClick={() => setView("card")}
              className={`flex items-center justify-center size-[26px] rounded-lg cursor-pointer transition-colors ${
                view === "card" ? "bg-white shadow-sm" : "text-text-muted"
              }`}
            >
              <LayoutGrid size={14} />
            </button>
            <button
              ...
              className={`flex items-center justify-center size-[26px] rounded-lg cursor-pointer transition-colors ${
                view === "list" ? "bg-white shadow-sm" : "text-text-muted"
              }`}
            >
              <List size={14} />
            </button>
          ...
          {/* Sort control */}
          <div className="flex items-center bg-[#f4f4f5] rounded-[10px] h-[32.5px] p-0.5">
            <button
              onClick={() => setSort("newest")}
              className={`flex items-center gap-1 h-[28px] px-2.5 rounded-lg text-[11px] font-medium cursor-pointer transition-colors ${
                sort === "newest" ? "bg-white shadow-sm text-text-primary" : "text-text-muted"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1 h-[28px] px-2.5 rounded-lg text-[11px] font-medium cursor-pointer transition-colors ${
                sort === "oldest" ? "bg-white shadow-sm text-text-primary" : "text-text-muted"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1 h-[28px] px-2.5 rounded-lg text-[11px] font-medium cursor-pointer transition-colors ${
                sort === "az" ? "bg-white shadow-sm text-text-primary" : "text-text-muted"
              }`}
            >
              <ArrowDownAZ size={12} />
              A-Z
      ...
      <div className={`px-section pt-2 pb-2${isEmpty ? " border-b border-border" : ""}`}>
        <div className="flex items-center gap-2 h-9 px-3 rounded-lg bg-[#f3f3f5] max-w-[448px]">
          <Search size={16} className="text-[#9f9fa9] shrink-0" />
          <input
            type="search"
            ...
            autoCapitalize="off"
            spellCheck={false}
            className="flex-1 text-[14px] text-text-primary bg-transparent outline-none placeholder:text-[#9f9fa9] [&::-webkit-search-cancel-button]:hidden"
          />
        </div>
      </div>
        ...
        <div className="flex items-center gap-2 px-section pt-2 pb-5 overflow-x-auto border-b border-border">
          <button
            ...
            className={`flex items-center gap-1.5 h-8 px-[13px] rounded-full text-[12px] font-medium cursor-pointer shrink-0 transition-colors border ${
              bookFilter === null
                ? "bg-[#faf5ff] border-[#e9d4ff] text-[#8200db]"
                : "bg-white border-[#e4e4e7] text-[#52525c] hover:bg-[#fafafa]"
            }`}
          >
            <BookOpen size={12} className={bookFilter === null ? "text-[#8200db]" : ""} />
            All Books
            <span className={`text-[11px] ${bookFilter === null ? "text-[#8200db]" : "text-text-muted"}`}>
              {words.length}
            </span>
          </button>
          {bookPills.map((pill) => (
            ...
            >
              <BookOpen size={12} className={bookFilter === pill.id ? "text-[#8200db]" : ""} />
              <span className="truncate max-w-[120px]">{pill.title}</span>
              <span className={`text-[11px] ${bookFilter === pill.id ? "text-[#8200db]" : "text-text-muted"}`}>
                {pill.count}
              </span>
            </button>
          ))}
      ...
      <div className="flex-1 overflow-auto px-section py-4">
        {isEmpty ? (
          ...
          <div className="flex flex-col items-center justify-center h-full">
            <div className="size-16 rounded-full bg-[#f4f4f5] flex items-center justify-center mb-4">
              <Languages size={28} className="text-[#9f9fa9]" />
            </div>
            <h2 className="text-[18px] font-medium text-text-primary mb-2">
              Start Building Your Vocabulary
            </h2>
            <p className="text-[14px] text-[#71717b] text-center max-w-[296px]">
            ...
            {groupedByLetter.map(([letter, letterWords]) => (
              <div key={letter} className="mb-6">
                ...
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-[18px] font-bold text-[#ad46ff]">{letter}</span>
                  <div className="flex-1 h-px bg-[#f4f4f5]" />
                  <span className="text-[11px] text-[#d4d4d8]">{letterWords.length}</span>
                </div>
                {/* Word rows */}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[13px] text-[#52525c] leading-5 truncate">
                          {defText}
                        </p>
                        {ctxText && (
                          <p className="text-[11px] italic text-[#9f9fa9] leading-4 truncate mt-0.5">
                            "{ctxText}"
                          </p>
                        )}
                      </div>
            ...
            {groupedByBook.map((group) => (
              <div key={group.title}>
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[11px] text-[#d4d4d8]">({group.words.length})</span>
                </div>
                {/* Word cards */}
                <div className="space-y-3">
                  {group.words.map((word) => {
                    ...
                    return (
                      <div
                        key={word.id}
                        className="group relative bg-[#fafafa] border border-[rgba(228,228,231,0.6)] rounded-[14px] p-[17px] flex flex-col gap-2"
                      >
                        {/* Trash — top right */}
                        <button
                          onClick={() => remove(word.id)}
                          className="absolute top-4 right-4 p-1 rounded hover:bg-white/80 cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                        ...
                        {/* Definition */}
                        <p className="text-[13px] text-[#52525c] leading-[20.15px] tracking-[-0.08px] line-clamp-3 w-[460px] max-w-full">
                          {defText}
                        </p>

                        {/* AI context — blockquote style */}
                        {ctxText && (
                          <div className="border-l-2 border-[#e9d4ff] pl-2 overflow-hidden">
                            <p className="text-[11px] italic text-[#9f9fa9] leading-[16.5px] tracking-[0.06px] line-clamp-2">
                              {ctxText}
                            </p>
                          </div>
                        )}
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            onClick={() => navigate(`/reader/${word.book_id}`, { state: { openVocab: true, cfi: word.cfi } })}
                            className="flex items-center gap-1 h-[24.5px] px-2.5 rounded-[10px] bg-[#faf5ff] text-[11px] font-medium text-[#9810fa] tracking-[0.06px] cursor-pointer hover:opacity-70"
                          >
                            Open in Reader
                            <ArrowRight size={12} />
                          </button>
